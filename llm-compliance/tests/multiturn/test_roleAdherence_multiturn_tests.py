...
def evaluate_rag_conversations():
    ...
    for test_case in test_cases:
        if not hasattr(test_case, 'chatbot_role') or not test_case.chatbot_role:
            test_case.chatbot_role = "You are a helpful AI assistant for students in a school. You maintain a helpful and educational tone."
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        if hasattr(test_case, 'chatbot_role') and test_case.chatbot_role:
            print(f"Chatbot Role: {test_case.chatbot_role[:100]}..." if len(test_case.chatbot_role) > 100 else f"Chatbot Role: {test_case.chatbot_role}")
    ...
    print("=" * 80)
    print("🔍 Running Role Adherence Evaluation")
    print("=" * 80)
    
    # Evaluate using the utility function
    results = evaluate_role_adherence_test_cases(
    ...
    for i, (test_case, score, reason, passed, error_msg) in enumerate(results, 1):
        ...
        if error_msg:
            ...
            print(f"Role Adherence Score: {score:.4f}" if score is not None else "Role Adherence Score: N/A")
