"""Test DeepEval RAG-specific multi-turn conversational metrics on conversational test cases.

This test file focuses on RAG-specific metrics that require retrieval context,
such as TurnFaithfulnessMetric, TurnContextualPrecisionMetric, TurnContextualRecallMetric,
and TurnContextualRelevancyMetric.
"""
...
from utils.deepEvalRAGMultiturnMetrics import (
    load_conversational_test_cases_from_json,
    verify_turn_faithfulness_metric,
    verify_turn_contextual_precision_metric,
    verify_turn_contextual_recall_metric,
    verify_turn_contextual_relevancy_metric
)

# Metric name to threshold path mapping (RAG-specific metrics only)
METRIC_THRESHOLD_MAP = {
    "Turn Faithfulness": ("deepeval", "multiturn_rag", "turn_faithfulness_min"),
    "Turn Contextual Precision": ("deepeval", "multiturn_rag", "turn_context_precision_min"),
    "Turn Contextual Recall": ("deepeval", "multiturn_rag", "turn_context_recall_min"),
    "Turn Contextual Relevancy": ("deepeval", "multiturn_rag", "turn_context_relevance_min"),
}

# Metric name to verification function mapping
METRIC_VERIFY_FUNC_MAP = {
    "Turn Faithfulness": verify_turn_faithfulness_metric,
    "Turn Contextual Precision": verify_turn_contextual_precision_metric,
    "Turn Contextual Recall": verify_turn_contextual_recall_metric,
    "Turn Contextual Relevancy": verify_turn_contextual_relevancy_metric,
}
...
def get_threshold(thresholds, metric_name):
    ...
    for key in path:
        if isinstance(value, dict) and key in value:
            ...
            return 0.5  # Default if path not found
...
def get_metric_model(thresholds, metric_name):
    ...
    model_map = {
        "Turn Faithfulness": ("deepeval", "multiturn_rag", "turn_faithfulness_model"),
        "Turn Contextual Precision": ("deepeval", "multiturn_rag", "turn_context_precision_model"),
        "Turn Contextual Recall": ("deepeval", "multiturn_rag", "turn_context_recall_model"),
        "Turn Contextual Relevancy": ("deepeval", "multiturn_rag", "turn_context_relevance_model"),
    }
...
def get_metric_mean_threshold(thresholds, metric_name):
    ...
    mean_map = {
        "Turn Faithfulness": ("deepeval", "multiturn_rag", "turn_faithfulness_mean"),
        "Turn Contextual Precision": ("deepeval", "multiturn_rag", "turn_context_precision_mean"),
        "Turn Contextual Recall": ("deepeval", "multiturn_rag", "turn_context_recall_mean"),
        "Turn Contextual Relevancy": ("deepeval", "multiturn_rag", "turn_context_relevance_mean"),
    }
...
def test_rag_multiturn_metrics(thresholds):
    """Test RAG-specific multi-turn conversational metrics on conversational test cases.
    
    This test focuses on RAG metrics that require retrieval_context in assistant turns:
    - TurnFaithfulnessMetric: Evaluates factual accuracy grounded in retrieval context
    - TurnContextualPrecisionMetric: Evaluates precision of retrieval context usage
    - TurnContextualRecallMetric: Evaluates recall of relevant information from context
    - TurnContextualRelevancyMetric: Evaluates relevancy of retrieval context
    """
    # Load conversational test cases
    # Default to RAG conversations with retrieval context
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        result_entry = {
             ...
             "has_retrieval_context": all(
                 hasattr(turn, 'retrieval_context') and turn.retrieval_context 
                 for turn in test_case.turns if turn.role == "assistant"
             ),
             "turns": [
                 {
                     "role": turn.role, 
                     "content": turn.content[:200] + "..." if len(turn.content) > 200 else turn.content,
                     "has_retrieval_context": hasattr(turn, 'retrieval_context') and bool(turn.retrieval_context) if turn.role == "assistant" else None
                 }
    ...
    if results:
        csv_file = reports_dir / f"multiturn_RAG_metrics_test_results_{timestamp}.csv"
    ...
    print(f"\n📊 RAG Multi-Turn Conversational Metrics Test Summary:")
    ...
    if metric_averages:
        summary_csv_file = reports_dir / f"rag_multiturn_metrics_summary_{timestamp}.csv"
