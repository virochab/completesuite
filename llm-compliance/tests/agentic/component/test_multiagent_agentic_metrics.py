"""Test multiple DeepEval agentic metrics on multi-agent system queries from taskCompletionData.jsonl.
...
# Initialize multi-agent client (module-level for reuse)
multi_agent_client = MultiAgentClient()


@observe(type="agent")
def multi_agent(input_query: str) -> tuple[str, Any]:
    """
    Multi-agent system wrapper function for DeepEval tracing with multiple metrics evaluation.
    ...
    Args:
        input_query: The user's query/question
    
    Returns:
        Tuple of (response_text, response_object):
        - response_text: The agent's response text (for evaluation)
        - response_object: The full response object (for accessing tools_called, trace, etc.)
    """
    # Execute the query by calling the multi-agent system
    response = multi_agent_client.ask(input_query)
...
def test_multiple_agentic_metrics(thresholds):
    ...
    on multi-agent system queries from taskCompletionData.jsonl.
    
    Uses EvaluationDataset and evals_iterator for evaluation, with comprehensive reporting.
    """
    # Load queries from taskMultiAgentCompletionData.jsonl
    queries_path = Path(__file__).parent.parent.parent.parent / "testData" / "agentic" / "taskMultiAgentCompletionData.jsonl"
    ...
    for query_case in queries:
        ...
        category = query_case.get("category", "multi_agent_query")
        ...
        multi_agent_client.reset_conversation(f"test-case-{case_id}")
        ...
        # evals_iterator evaluates metrics automatically after multi_agent is called
        for golden in temp_dataset.evals_iterator(metrics=[task_completion, step_efficiency, plan_adherence, plan_quality]):
            try:
                # Call the multi-agent system with the query
                ...
                output, response = multi_agent(golden.input)
                ...
                error_msg = f"Error calling multi_agent for case {case_id}: {e}"
        ...
        # Optional: Verify expected tool(s) were called
        if expected_tool:
            ...
            # Handle both single tool (string) and multiple tools (list)
            if isinstance(expected_tool, list):
                # Multiple expected tools - check that all are called
                missing_tools = [tool for tool in expected_tool if tool not in tool_names]
                if missing_tools:
                    failures.append(
                        f"Expected tools {expected_tool} not all called for case {case_id}. "
                        f"Missing: {missing_tools}. Tools called: {tool_names}"
                    )
            else:
                # Single expected tool - check if it's called
    ...
    print("\n" + "="*60)
    print("MULTIPLE AGENTIC METRICS RESULTS - MULTI-AGENT SYSTEM")
    print("="*60)
