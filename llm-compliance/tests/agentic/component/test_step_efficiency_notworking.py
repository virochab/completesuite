"""Not working Test DeepEval StepEfficiencyMetric on weather agent queries from taskCompletionData.jsonl."""
...
from utils.deepComponentAgenticMetrics import verify_step_efficiency_metric
...
def weather_agent(input_query: str) -> tuple[str, Any]:
    ...
    3. Provide execution context to StepEfficiencyMetric for evaluation
    
    For StepEfficiencyMetric, we need to call update_current_trace() inside
    the @observe() decorated function to properly set up the trace context.
    ...
    # Update current trace for StepEfficiencyMetric
    # This must be called inside the @observe() decorated function
    # StepEfficiencyMetric uses trace information to count steps and evaluate efficiency
    # Note: Some versions of StepEfficiencyMetric may require 'prompt' parameter
    tools_called = response.tools_called if hasattr(response, 'tools_called') else []
    try:
        # Try with prompt parameter (some versions may require it)
        update_current_trace(
            input=input_query,
            output=response.text,
            tools_called=tools_called,
            prompt=input_query  # Add prompt parameter for StepEfficiencyMetric
        )
    except TypeError:
...
def test_step_efficiency_metrics(thresholds):
    """Test StepEfficiencyMetric on queries from taskCompletionData.jsonl."""
    # Load step efficiency queries (using same data as task completion)
    queries_path = Path(__file__).parent.parent.parent.parent / "testData" / "agentic" / "taskCompletionData.jsonl"
    
    if not queries_path.exists():
        pytest.skip(f"Step efficiency queries file not found: {queries_path}")
    ...
    if not queries:
        pytest.skip("No step efficiency queries found in file")
    ...
    log_file = evidence_dir / f"step_efficiency_test_log_{timestamp}.jsonl"
    
    # Get threshold from config (default to 0.7 if not found)
    step_efficiency_threshold = thresholds.get("deepeval", {}).get("agentic", {}).get("step_efficiency_min", 0.7)
    ...
    for i, query_case in enumerate(queries):
        ...
        try:
            ...
            # Evaluate step efficiency using the utility function
            # StepEfficiencyMetric evaluates whether the task was completed efficiently
            # (using minimum number of steps/tool calls)
            score, reason, passed, error_msg = verify_step_efficiency_metric(
                actual_output=output,
                input_query=query,
                tools_called=tools_called,  # Pass tools_called for step efficiency evaluation
                threshold=step_efficiency_threshold,
                model='gpt-4o',
                include_reason=True,
                case_id=case_id
            )
            ...
            if not passed:
                failures.append(
                    f"StepEfficiency score={score:.4f} < threshold {step_efficiency_threshold} "
                    f"for case {case_id}"
                )
            ...
            error_msg = f"Error evaluating step efficiency for case {case_id}: {e}"
    ...
    if results:
        csv_file = reports_dir / f"step_efficiency_test_results_{timestamp}.csv"
    ...
    print(f"\n📊 Step Efficiency Metric Test Summary:")
    ...
    print("\n" + "="*60)
    print("STEP EFFICIENCY METRIC RESULTS - WEATHER AGENT")
    print("="*60)
