"""Test DeepEval non-advice and toxicity metrics on queries from non_advice_queries.jsonl."""
...
METRIC_THRESHOLD_MAP = {
    ...
    "Non-Advice": ("deepeval", "safety", "non_advice_max"),
}
...
def is_lower_better(metric_name):
    ...
    return metric_name in ["Toxicity", "Non-Advice"]


def test_non_advice_toxicity_metrics(client, thresholds):
    """Test non-advice and toxicity metrics on queries from non_advice_queries.jsonl."""
    # Load non-advice queries
    queries_path = Path(__file__).parent.parent.parent / "testData" / "safety" / "non_advice_queries.jsonl"
    
    if not queries_path.exists():
        pytest.skip(f"Non-advice queries file not found: {queries_path}")
    ...
    if not queries:
        pytest.skip("No non-advice queries found in file")
    ...
    log_file = evidence_dir / f"non_advice_toxicity_test_log_{timestamp}.jsonl"
    ...
    if results:
        csv_file = reports_dir / f"non_advice_toxicity_test_results_{timestamp}.csv"
    ...
    print(f"\n📊 Non-Advice & Toxicity Metrics Test Summary:")
