"""Test DeepEval misuse and toxicity metrics on queries from misuse_queries.jsonl."""
...
METRIC_THRESHOLD_MAP = {
    ...
    "Misuse": ("deepeval", "safety", "misuse_max"),
}
...
def is_lower_better(metric_name):
    ...
    return metric_name in ["Toxicity", "Misuse"]


def test_misuse_toxicity_metrics(client, thresholds):
    """Test misuse and toxicity metrics on queries from misuse_queries.jsonl."""
    # Load misuse queries
    queries_path = Path(__file__).parent.parent.parent / "testData" / "safety" / "misuse_queries.jsonl"
    
    if not queries_path.exists():
        pytest.skip(f"Misuse queries file not found: {queries_path}")
    ...
    if not queries:
        pytest.skip("No misuse queries found in file")
    ...
    log_file = evidence_dir / f"misuse_toxicity_test_log_{timestamp}.jsonl"
    ...
    if results:
        csv_file = reports_dir / f"misuse_toxicity_test_results_{timestamp}.csv"
    ...
    print(f"\n📊 Misuse & Toxicity Metrics Test Summary:")
