"""Test DeepEval role violation and toxicity metrics on queries from role_violation_queries.jsonl."""
...
METRIC_THRESHOLD_MAP = {
    ...
    "Role Violation": ("deepeval", "safety", "role_violation_max"),
}

# GEval criteria for supporting metrics
METRIC_CRITERIA = {
}
...
def is_lower_better(metric_name):
    """Check if lower scores are better for a metric."""
    return metric_name in ["Toxicity", "Role Violation"]


def test_role_violation_toxicity_metrics(client, thresholds):
    """Test role violation and toxicity metrics on queries from role_violation_queries.jsonl."""
    # Load role violation queries
    queries_path = Path(__file__).parent.parent.parent / "testData" / "safety" / "role_violation_queries.jsonl"
    
    if not queries_path.exists():
        pytest.skip(f"Role violation queries file not found: {queries_path}")
    ...
    if not queries:
        pytest.skip("No role violation queries found in file")
    ...
    log_file = evidence_dir / f"role_violation_toxicity_test_log_{timestamp}.jsonl"
        ...
        for metric_name in metrics_to_test:
            try:
                ...
                if metric_name == "Toxicity":
                    ...
                    # Use GEval for supporting metrics
    ...
    if results:
        csv_file = reports_dir / f"role_violation_toxicity_test_results_{timestamp}.csv"
    ...
    total_metrics_tested = len(queries) * len(METRIC_THRESHOLD_MAP)
    total_failures = len(failures)
    
    print(f"\n📊 Role Violation & Toxicity Metrics Test Summary:")
