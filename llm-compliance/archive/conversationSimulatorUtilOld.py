...
from utils.wrapModelForSyntheticData import model_callback

# Example weather-related ConversationalGolden
conversation_golden = ConversationalGolden(
    scenario="A user is planning a trip to New York next week and wants to know the weather forecast to decide what to pack.",
    expected_outcome="The user receives accurate weather information for New York for the upcoming week and gets recommendations on what to pack based on the forecast.",
    user_description="A traveler planning a week-long trip to New York.",
)
...
def save_conversational_test_cases_to_json(test_cases, output_dir: str, filename_prefix: str = "conversational_test_cases"):
    ...
    print(f"\n✅ Saved {len(test_cases)} conversational test cases to: {filepath}")
    return filepath
...
print("=== Simulating conversation with example weather scenario ===")
...
output_dir = project_root / "utils" / "synthetic_data" / f"conversations_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
#         str(output_dir_json),
#         "weather_conversations_from_json"
#     )